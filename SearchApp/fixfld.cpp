...
int FixedFieldBuffer::AddField(int fieldSize)
{
	...
	this->FieldSize[NumFields] = fieldSize;
	this->NumFields++;
...
// return number of bytes packed, -1 if error
{
	...
	if (this->NextField == this->NumFields || !this->Packing) // buffer is full or not packing mode
		return -1;
	int start = this->NextByte; // first byte to be packed
	int packSize = FieldSize[NextField]; // number bytes to be packed
	if (size != -1 && packSize != size) return -1;
	memcpy(&this->Buffer[start], field, packSize); // move bytes to buffer
	this->NextByte += packSize;
	this->NextField++;
	if (this->NextField == this->NumFields) // all fields packed
	{
		this->Packing = -1;
